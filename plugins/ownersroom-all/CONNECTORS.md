...
The MCP server also exposes **25 read-only Resources** — passive context the LLM can browse without burning a tool call. Resource-aware clients (Claude Code, Claude Desktop, Gemini CLI) auto-discover these via `resources/list` and `resources/templates/list`. Resources don't replace tools; tools still take parameters and perform writes.

Each persona profile exposes a tailored subset; the kitchen-sink (`ownersroom-all`) shows all 25.
